...
import { useCreateClientMutation, useUpdateClientMutation } from '@/api/clients/mutations';
...
watch(() => clientQuery.data.value, (c) => {
  if (!c) return;
...
<template>
  <div class="client-form">
    <a-card :title="isEditMode ? ($t('clients.editTitle') || 'Edit Client') : ($t('clients.newTitle') || 'New Client')">
      <a-form layout="vertical" :model="formState" @finish="submit">
        <a-row :gutter="16">
          <a-col :xs="24" :md="12">
            <a-form-item :label="$t('clients.name') || 'Name'" name="name" :rules="[{ required: true, message: $t('validation.required') }]">
              <a-input v-model:value="formState.name" />
            </a-form-item>
          </a-col>
          <a-col :xs="24" :md="12">
            <a-form-item :label="$t('clients.email') || $t('auth.email')" name="email">
              <a-input v-model:value="formState.email" />
            </a-form-item>
          </a-col>
          <a-col :xs="24" :md="12">
            <a-form-item :label="$t('clients.phone') || 'Phone'" name="phone">
              <a-input v-model:value="formState.phone" />
            </a-form-item>
          </a-col>
          <a-col :xs="24" :md="12">
            <a-form-item :label="$t('clients.address') || 'Address'" name="address">
              <a-input v-model:value="formState.address" />
            </a-form-item>
          </a-col>
          <a-col :xs="24">
            <a-form-item :label="$t('clients.notes') || 'Notes'" name="notes">
        ...
        <a-space>
          ...
          <a-button type="primary" html-type="submit">{{ isEditMode ? $t('common.update') : $t('common.create') }}</a-button>
        </a-space>
      </a-form>
    </a-card>
  </div>
