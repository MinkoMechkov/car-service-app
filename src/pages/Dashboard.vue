<script setup lang="ts">
  ...
  import {
    ...
    useClientRepairsQuery,
  } from '@/api/repairs/queries';
  import { useVehiclesQuery, useClientVehiclesQuery } from '@/api/vehicles/queries';
  import type { RecentRepair } from '@/api/repairs/interfaces';
  ...
  const activeRepairs = computed(() => {
    return repairsQuery.data.value?.filter((r) => getStatus(r) === 'in_progress').length ?? 0;
  });

  const completedToday = computed(() => {
    const today = dayjs();
    return (
      repairsQuery.data.value?.filter((r) => dayjs(r.date).isSame(today, 'day') && r.total_cost)
        .length ?? 0
    );
  });
  ...
  const thisMonthRevenue = computed(() => {
    ...
    return (
      repairsQuery.data.value
        ?.filter(
          (r) => dayjs(r.date).isAfter(start.subtract(1, 'day')) && dayjs(r.date).isBefore(end)
        )
...
<template>
  <div class="dashboard-container">
    ...
    <!-- Stats Cards -->
    <a-row v-if="isAdmin" :gutter="[16, 16]" class="stats-row">
        ...
        <a-card class="stat-card stat-card-green" :bordered="false">
          <div class="stat-content">
            ...
            <div class="stat-info">
              ...
              <div class="stat-trend positive">
                <ArrowUpOutlined />
                <span>8% {{ $t('dashboard.fromLastWeek') }}</span>
              </div>
            </div>
          </div>
        </a-card>
        ...
        <a-card class="stat-card stat-card-purple" :bordered="false">
          <div class="stat-content">
            ...
            <div class="stat-info">
              ...
              <div class="stat-trend positive">
                <ArrowUpOutlined />
                <span>15% {{ $t('dashboard.fromLastMonth') }}</span>
              </div>
            </div>
          </div>
        </a-card>
          ...
          <a-card class="stat-card stat-card-orange" :bordered="false">
            <div class="stat-content">
              ...
              <div class="stat-info">
                ...
                <div class="stat-trend positive">
                  <ArrowUpOutlined />
                  <span>5% {{ $t('dashboard.fromLastMonth') }}</span>
                </div>
              </div>
            </div>
          </a-card>
          ...
          <a-card class="stat-card stat-card-teal" :bordered="false">
            <div class="stat-content">
              ...
              <div class="stat-info">
                ...
                <div class="stat-trend positive">
                  <ArrowUpOutlined />
                  <span>3% {{ $t('dashboard.fromLastMonth') }}</span>
                </div>
              </div>
            </div>
          </a-card>
        ...
        <router-link to="/vehicles">
          <a-card class="stat-card stat-card-teal" :bordered="false">
            <div class="stat-content">
              ...
              <div class="stat-info">
                <p class="stat-label">{{ $t('dashboard.myVehicles') }}</p>
                <h2 class="stat-value">{{ totalVehicles }}</h2>
              </div>
            </div>
          </a-card>
        ...
        <router-link to="/repairs">
          <a-card class="stat-card stat-card-blue" :bordered="false">
            <div class="stat-content">
              ...
              <div class="stat-info">
                <p class="stat-label">{{ $t('dashboard.myRepairs') }}</p>
                <h2 class="stat-value">{{ repairsQuery.data.value?.length ?? 0 }}</h2>
              </div>
            </div>
          </a-card>
        </router-link>
            ...
            <template #bodyCell="{ column, record }">
              <template v-if="column.key === 'vehicle'">
                <div class="vehicle-info">
                  ...
                  <span class="vehicle-plate">{{ record.plate }}</span>
                </div>
              </template>
    ...
    <!-- Revenue Chart -->
    <a-row v-if="isAdmin" :gutter="[16, 16]">
      <a-col :xs="24">
        <a-card
