...
export function useUpdateOffer() {
  ...
  return useMutation({
    mutationFn: async ({ 
      ...
      parts?: Array<{ name: string; part_id?: string; quantity: number; price: number }>;
      services?: Array<{ name: string; service_id?: string; price: number }>;
    }) => {
    ...
    onSuccess: (_, variables) => {
      queryClient.invalidateQueries({ queryKey: ['adminOffersList'] });
      queryClient.invalidateQueries({ queryKey: ['offerDetails', variables.offerId] });
    },
  });
}
